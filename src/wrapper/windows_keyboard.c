...
int get_scancode(WPARAM vkCode, LPARAM lParam) {
    ...
    if (vkCode >= 'A' && vkCode <= 'Z') {
        ...
        int index = vkCode - 'A';
        int scancode = letterScancodes[index];
        
        if (alt) return (scancode << 8);
        if (ctrl) return (scancode << 8) | (index + 1);
        if (shift) return (scancode << 8) | (vkCode); // Uppercase
        return (scancode << 8) | (vkCode + 32); // Lowercase
    }
    
    // Number keys (0-9)
    if (vkCode >= '0' && vkCode <= '9') {
        const int numberScancodes[] = {
        ...
        int index = vkCode - '0';
        int scancode = numberScancodes[index];
        
        if (alt) return (scancode + 0x76) << 8;
        if (vkCode == '2' && ctrl) return 0x0300;
        if (vkCode == '6' && ctrl) return 0x071E;
        if (shift) return (scancode << 8) | shiftedNumbers[index];
        return (scancode << 8) | vkCode;
    }
    
    // Function keys (F1-F12)
    if (vkCode >= VK_F1 && vkCode <= VK_F12) {
        int fnum = vkCode - VK_F1;
        const int functionScancodes[] = {
            0x3B, 0x3C, 0x3D, 0x3E, 0x3F, 0x40, 0x41, 0x42, 0x43, 0x44, 0x85, 0x86
        };
        
        int scancode = functionScancodes[fnum];
        
        if (alt) return ((scancode + 0x2D) << 8);
        if (ctrl) return ((scancode + 0x23) << 8);
        if (shift) return ((scancode + 0x19) << 8);
        return (scancode << 8);
    }
    ...
    switch (vkCode) {
        ...
        // Symbol keys - these need special handling based on OEM scan codes
        case VK_OEM_MINUS: // -_
